...
# Data
parser.add_argument('--train-root', type=str, default='./SFEWdataset/images/train',
                    help="root path to train data directory")
parser.add_argument('--test-root', type=str, default='./SFEWdataset/images/test',
                    help="root path to test data directory")
parser.add_argument('--label-train', default='./SFEWdataset/labels/SFEW_train_label2.txt',
                    type=str, help='')
parser.add_argument('--label-test', default='./SFEWdataset/labels/SFEW_test_label2.txt',
                    type=str, help='')
...
def train(labeled_trainloader, unlabeled_trainloader, model, model2, optimizer, optimizer2, criterion_ce,
    ...
    for batch_idx in range(args.train_iteration):
        ...
        if epoch > 5:
          ...
          # logits_unlabeled = torch.zeros((2,400))
          ...
          p_fussion = 1.0 * outputs_u + 0.4 * simmilarity2 + 0.2 * p_unlabeled_3
        else:
          p_fussion = 0.7 * outputs_u + 0.3 * simmilarity2 
        ...
        loss = 0.7 * Ls + 0.2 * L_text + 0.1 * L_con
                ...
                if epoch > 10:
                  ...
                  for i in range(batchsize_text):
                      for j, label in enumerate(global_labels):
                          p_unlabeled_3[i, label] = torch.max(p_unlabeled_3[i, label], logits_unlabeled[i, j])
                  p_unlabeled_3 = torch.softmax(p_unlabeled_3, dim=-1)
                  p_fussion = 1.0 * p_unlabeled_1 + 1.0 * p_unlabeled_2 + 1.0 * p_unlabeled_3
                else: p_fussion = 1.0 * p_unlabeled_1 + 1.0 * p_unlabeled_2
                p_fussion_normalized = F.softmax(p_fussion, dim=1)
                # max_probs, max_idx = torch.max(p_fussion, dim=1)
                prec1, prec5 = accuracy(p_fussion_normalized, targets, topk=(1, 5))
